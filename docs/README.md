...
To build Almanac from source, make sure you have `go`{.verbatim}
installed with version \>= 1.18.1.
...
Next `cd`{.verbatim} in to the directory.
...
This will create a binary in the `bin`{.verbatim} directory, from there
you should move it to somewhere on your `$PATH`{.verbatim}.
